...
let btnInput = document.getElementById("btnInputSearch");
...
async function weather(item) {
  let dataBecome = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=32d9b51efba849fd852205358241512&q=${item}&days=3&aqi=yes&alerts=no`
  );
  let data = await dataBecome.json();
  currentWeather(data);
...
function currentWeather(data) {
  ...
  const dayNumber = new Date(data.current.last_updated).getDate();
  const monthName = todayDate.toLocaleDateString("en-US", { month: "long" });
  let cardWeather = ` <div class="card card-bg border-0">
            <div
  ...
  weatherContainer.innerHTML = cardWeather;
}
function forcasteWeather(data) {
  let forcastdayInfo = data.forecast.forecastday;
  let comingDays = "";
  for (let i = 1; i < forcastdayInfo.length; i++) {
    let todayDate = new Date(forcastdayInfo[i].date);
    let dayName = todayDate.toLocaleDateString("en-US", { weekday: "long" });
    let cardClass = i % 2 === 0 ? "card-bg" : "card-bg-sec";
    comingDays += `
  ...
  const weatherContainerTwo = document.getElementById("weather-container");
  weatherContainerTwo.innerHTML += comingDays;
}
